...
def extract_tree_levels(tree_number):
    ...
    # splits tree number according to different identifier codes
...
def parse_mesh_terms(xml_path):
    ...
    A deterministic canonical Tree Number is inherited by every term from their descriptor.
    All Tree Numbers are added as metadata
    """

    rows = []

    # iterparse reads the XML step by step
    for _, elem in ET.iterparse(xml_path, events=("end",)):
        ...
        # extracts tree numbers of a descriptor 
        # and sorts them according to size in ascending order
        ...
        # writes context information as a json
        metadata_json = json.dumps(extract_metadata(elem), 
                                   ensure_ascii=False, separators=(",", ":"))

        # extracts concept information
        for concept in elem.findall("ConceptList/Concept"):
            ...
            for term in concept.findall("TermList/Term"):
                ...
                # if TermUI or Term String do not exist skipped.
...
def calculate_item_size(row):
    """
    Calculates the item size by bytes in UTF-8 for a row.
    """
...
def process_mesh_terms(input_path, output_path):
    ...
    # if the DataFrame of parsed mesh terms is smaller than SOURCE_ROWS allows -> error

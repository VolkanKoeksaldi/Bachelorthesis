...
def select(search, config, node_id, placement_type, table, where=None, parameters=()):
    ...
    search:
        Columns that are selected by the query, for example "descriptor_ui" or "*"

    node_id:
        ID of the node to be queried, for example "node_1"
    
    placement_type:
        "round_robin", "tuple_ilp" or "conflict_locality_ilp"
    
    table:
        Database table to query
    
    where:
        Optional WHERE condition with "?" placeholders, for example "descriptor_ui = ?"
    
    parameters:
        Values used for placeholders in WHERE condition
    """
...
def insert_into_node(item_id, item_name, fragment_ids, node_id, placement_type, config):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for fragment_id in fragment_ids:
            # Checks whether fragment is stored on this node
            cur.execute(
                """
                SELECT *
                FROM fragments
            ...
            if cur.fetchone() is None:
                raise ValueError(
                    f"Fragment {fragment_id} is not stored on node {node_id}."
                )
...
def validate_updates(table, updates, config):
    ...
    if not updates or not updates.strip():
        raise ValueError("No SET clause was specified.")
    ...
    for assignment in assignment_array:
        ...
        if placeholders != "?":
            raise ValueError(f"The SET assignment for {column} does not have a ? placeholder.")

        if column in validated:
            raise ValueError(f"Column {column} was specified more than once.")
...
def update(node_id, config, placement_type, table, updates, where=None, parameters=()):
    ...
    updates:
        String with column names and placeholders
        Example: "descriptor_name = ?, fragment_size = ?"
    
    where:
        Optional WHERE condition with placeholders
        Example: "descriptor_ui = ?"
    ...
    if not updates:
        raise ValueError(f"No update values were given.")
...
def validate_delete(table, where, config):
    """
    Validates WHERE condition of a DELETE query.
    ...
    for condition in condition_array:
        ...
        if len(condition_part) != 2:
            raise ValueError(f"Invalid WHERE condition with: {condition}")
...
def delete(node_id, config, placement_type, table, where=None, parameters=(), delete_all=False):
    ...
    where:
        WHERE conditions using placeholders
        Example "descriptor_ui = ?".
